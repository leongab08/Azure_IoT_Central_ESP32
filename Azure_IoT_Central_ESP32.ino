...
// SPDX-License-Identifier: MIT okey logrado
...
#define SERVO_PIN 26 // Pin donde está conectado el servomotor
#define LIGHT_SENSOR_ADDRESS 0x23 // Dirección I2C del BH1750
#define LIGHT_THRESHOLD 2000 // Umbral de luminosidad

Servo servoMotor;

// ** Configuración del ventilador **
#define FAN_PIN 27 // Pin conectado al ventilador (L298N OUT1 y OUT2)
#define TEMPERATURE_THRESHOLD 25.0 // Temperatura máxima para activar el ventilador (en °C)



// ** Configuración de la bomba de agua **
#define IN1_PIN 25  // Pin conectado a IN1 del L298N
#define IN2_PIN 33  // Pin conectado a IN2 del L298N
#define HUMIDITY_THRESHOLD 45 // Umbral de humedad mínima (en %)

bool isWatering = false; // Estado de la bomba
...
void setup() {
  ...
  servoMotor.write(0); // Inicialmente cerrar la compuerta
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    switch (azure_iot_get_status(&azure_iot)) {
      case azure_iot_connected:
        ...
        if (send_device_info) {
          if (azure_pnp_send_device_info(&azure_iot, properties_request_id++) == 0) {
            send_device_info = false;
          }
        } else {
          if (azure_pnp_send_telemetry(&azure_iot) != 0) {
            Serial.println("Error al enviar telemetría.");
          }
        }
        break;
  ...
  // Control del servomotor basado en la luminosidad
  ...
  // Control de la bomba de agua basado en la humedad
  int rawMoisture = analogRead(soilMoisturePin);
  int moistureLevel = map(rawMoisture, 3000, 1000, 0, 100);
  ...
  if (!isWatering && moistureLevel < HUMIDITY_THRESHOLD) {
    digitalWrite(IN1_PIN, HIGH);
    digitalWrite(IN2_PIN, LOW);
    isWatering = true;
    wateringStartTime = millis();
  } else if (isWatering && millis() - wateringStartTime >= 2000) {
    digitalWrite(IN1_PIN, LOW);
    digitalWrite(IN2_PIN, LOW);
    isWatering = false;
  }

  delay(1000); // Intervalo entre tareas
}
